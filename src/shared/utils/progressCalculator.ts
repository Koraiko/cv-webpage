...
// TODO: P50 - Copilot helped with calculateProgress bug => maybe refactor this after understanding the logic/ math
/**
...
export class ProgressCalculator {
	...
	calculateProgress(currentSection: CurrentSection, scrollTop: number): number {
		...
		const itemProgress =
			((2 * currentSectionIndex + 1) / (2 * totalItems)) * 100;
		...
		const nextItemProgress =
			((2 * (currentSectionIndex + 1) + 1) / (2 * totalItems)) * 100;
		const progressIncrement =
			(nextItemProgress - itemProgress) * sectionProgress;
